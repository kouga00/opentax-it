...
4. Ricevute: lettura IMAP (pulsante "Controlla ricevute", controllo periodico con l'app aperta); riconoscimento delle ricevute SDI (consegna RC, scarto NS, mancata consegna MC; DT solo per la PA) e delle ricevute del gestore PEC (accettazione e consegna attestano la trasmissione, non l'emissione); aggiornamento dello stato della fattura; salvataggio dell'indirizzo PEC assegnato; archiviazione delle ricevute nello storage.
   **Riconciliazione (l'app può restare spenta per giorni)**. Le ricevute non si perdono: lo SDI le inoltra "sullo stesso canale attraverso il quale sono state trasmesse le relative fatture" (Spec. 1.9.1 §1.5), cioè nella casella PEC, dove restano. Quindi niente ascolto in tempo reale, ma una sincronizzazione idempotente con la casella:
   ...
   - **Invio rimasto "in invio"**: se l'API cade dopo l'invio SMTP e prima di salvare lo stato, `SdiTransmission` resta `PENDING` e la fattura `SENT`. Dopo qualche minuto la sincronizzazione cerca nella casella la ricevuta di accettazione del gestore o una ricevuta SDI con quel nome file; se non trova niente lo segnala all'utente, senza reinviare da sola (meglio chiedere che rischiare un doppio invio). **Da verificare** come le ricevute di accettazione e consegna del gestore richiamano il messaggio originale: regole tecniche PEC (DM 2 novembre 2005), non ancora lette né archiviate.
