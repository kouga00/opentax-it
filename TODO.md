...
- **Pattern API**: rivedere percorsi, metodi e codici di stato (es. `POST /fiscal-rules/seed`, azioni come `/issue` e `/activate`) e scegliere una convenzione unica, documentata in AGENTS.md.
  ...
  - Risorse da separare, emerse dalla revisione degli endpoint del 26/09/2026:
    ...
    - `taxes`: i crediti d'imposta diventano un modulo proprio (`/tax-credits`), `taxes` resta per riepilogo e dati dell'anno;
    ...
    - `f24`: i piani di rateazione (`/f24/plans/:taxYear`) separati dai modelli F24 (`/f24/:id`), in controller distinti;
    - `invoices`: import, documenti (XML, PDF, anteprima) e soglie in controller distinti; le soglie dell'anno (`GET /invoices/thresholds`) sono sugli incassi, quindi vanno con le imposte;
    - `payments`: `GET /invoices/:id/payments` non è più usato dal web, perché `/collection` restituisce anche gli incassi: da togliere.
