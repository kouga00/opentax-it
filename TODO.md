...
- **Cartelle di ogni modulo** (AGENTS.md): fatto il 26/09/2026 per `customers`, `f24`, `invoices`, `payments`, `tax-credits` e `taxes`; `CreateCustomerDto` e `UpdateCustomerDto` erano la stessa classe, ora `SaveCustomerDto`. Da fare dopo il merge della PR #12, che tocca gli stessi file: `tenants`, `fiscal-rules`, `sources`, `exchange-rates` e i moduli nuovi della PR (`auth`, `audit-log`).
...
- **Pattern API**: rivedere percorsi, metodi e codici di stato (es. `POST /fiscal-rules/seed`, azioni come `/issue` e `/activate`) e scegliere una convenzione unica, documentata in AGENTS.md.
  - Fatto il 26/09/2026: `GET /invoices/:id/collection` (totale, incassato, residuo, modalità e incassi, con DTO di risposta e mapper), usato dalla modale di incasso e dal dettaglio.
  - Fatto il 26/09/2026, con DTO di richiesta e di risposta (uno per file), tipi in `types/`, mapper e Swagger sugli endpoint:
    - `payments`: tolti `GET /invoices/:id/payments` e `GET /payments?year=`, non usati dal web;
    - `tax-credits`: modulo proprio, `/tax-credits` al posto di `/taxes/credits`;
    - `f24`: `F24Controller` per i modelli (`/f24`) e `InstallmentPlansController` per i piani (`/installment-plans/:taxYear`, al posto di `/f24/plans/:taxYear`, dove `plans` si confondeva con un id);
    - `invoices`: controller distinti per documenti, import (`/invoices/import`), file (`/invoices/:id/xml` e `/pdf`) e soglie dell'anno (`/revenue-thresholds`, al posto di `/invoices/thresholds`); tolto `GET /invoices/:id/preview`, non usato. Le risposte non contengono più `tenantId`, il percorso dell'XML nello storage e l'anagrafica completa del cliente. `CreateInvoiceDto` e `UpdateInvoiceDto` erano la stessa classe: ora `SaveInvoiceDto`.
  - Da fare dopo il merge della PR #12 (autenticazione), che tocca gli stessi file:
...
**Piano (canale PEC)**
Fatto il 26/09/2026 (punti 1-3 e 7): cifratura AES-256-GCM della password PEC (`common/secret-cipher.ts`), mai restituita dall'API; configurazione PEC in Impostazioni con gestori preconfigurati (`packages/fatturapa/src/sdi-pec.ts`: Aruba, Poste, Postel, InfoCert; Register.it e Namirial come "Altro gestore") e prova di connessione SMTP/IMAP; pulsante "Invia allo SDI" con conferma, invio SMTP a `sdi01@pec.fatturapa.it` o all'indirizzo assegnato, registrazione in `SdiTransmission`, errori senza dettagli interni e fattura riportata a "emessa" se l'invio fallisce. Le fatture importate e quelle verso la PA non si inviano. Finché le ricevute non si leggono (punto 4), l'indirizzo assegnato dallo SDI si inserisce a mano in Impostazioni e dopo il primo invio i successivi sono bloccati senza. Nessuna prova reale del canale ancora fatta (punto 8).
1. ~~Cifratura a riposo delle credenziali~~ fatto.
2. ~~Configurazione PEC in Impostazioni~~ fatto: indirizzo, server SMTP e IMAP, credenziali; prova di connessione senza inviare nulla.
   - **Gestori preconfigurati**: una tendina con i parametri già compilati, così l'utente inserisce solo indirizzo e password. Il gestore deve essere nell'elenco pubblico AgID (Specifiche tecniche SdI, Allegato B DM 55/2013, v1.8.4 del 31/03/2026, §3.1.1). Parametri trovati il 26/09/2026 sulle pagine ufficiali dei gestori, tutti con SSL/TLS implicito (IMAP 993, SMTP 465):
     - Aruba PEC: `imaps.pec.aruba.it` e `smtps.pec.aruba.it` ([guida](https://guide.aruba.it/pec/configurazione-programmi-di-posta/client-posta-e-dispositivi-mobili));
     - Poste Italiane (`postecert.it`): `mail.postecert.it` ([pagina](https://postecert.poste.it/pec/configurazione.shtml));
     - Postel (`postecertifica.it`): `mail.postecertifica.it`, con una password per il client generata a parte, che scade ogni 90 giorni ([manuale operativo 1.2](https://www.media.poste.it/b22c9cf8-0d42-47ce-808a-205c84215064/file/pec_postecertifica_manuale_operativo));
     - InfoCert Legalmail: `mbox.cert.legalmail.it` e `sendm.cert.legalmail.it`, con lo User ID al posto dell'indirizzo ([guida](https://help.infocert.it/home/guida/guida-alla-configurazione-legalmail---apple-mail?lang=it));
     ...
     - Namirial e "Altro gestore": inserimento manuale. Gli host di Namirial non sono su una pagina ufficiale raggiungibile: la sua guida rimanda alle impostazioni della webmail.

     I parametri vanno in un file con la fonte di ogni gestore e la data di verifica, come per le regole fiscali, e ricontrollati periodicamente.

     **Verifica in due passaggi e programmi di posta** (verificato il 26/09/2026 sulle pagine ufficiali; detto nel form sotto il campo password):
     - Aruba: con la verifica attiva "i programmi di posta [...] non possono più utilizzare la password principale della PEC"; serve la "Password per programmi di posta" da Gestione account PEC → Sicurezza, valida 6 mesi e prorogabile ([cos'è](https://guide.aruba.it/pec/account-e-sicurezza/verifica-in-2-passaggi/cos-e), [come generarla](https://guide.aruba.it/pec/account-e-sicurezza/password/generare-password-programmi-posta)).
     - Postel (postecertifica.it): accesso con autenticazione forte in webmail; per i client la password si genera dalla dashboard ("Genera password") e scade ogni 90 giorni (manuale operativo 1.2).
...
3. ~~Invio~~ fatto: pulsante "Invia allo SDI" sulla fattura emessa; SMTP verso `sdi01@pec.fatturapa.it` o l'indirizzo assegnato; registrazione in `SdiTransmission`; stato "inviata".
4. ~~Ricevute~~ fatto il 26/09/2026: lettura IMAP (pulsante "Controlla ricevute", controllo periodico con l'app aperta); riconoscimento delle ricevute SDI (consegna RC, scarto NS, mancata consegna MC; DT solo per la PA) e delle ricevute del gestore PEC (accettazione e consegna attestano la trasmissione, non l'emissione); aggiornamento dello stato della fattura; salvataggio dell'indirizzo PEC assegnato; archiviazione delle ricevute nello storage.
   Fatto: lettura della casella in sola lettura (EXAMINE e BODY.PEEK, imapflow) dall'ultimo UID salvato, rilettura per data se cambia `UIDVALIDITY`; messaggi PEC letti con postal-mime (mailparser è in manutenzione e consiglia PostalMime); ricevute del gestore collegate con `X-Riferimento-Message-ID` o `msgid` di `daticert.xml` e, in mancanza, con l'oggetto; ricevute SDI (RC, NS, MC) lette da `postacert.eml` sui soli messaggi il cui mittente certificato in `daticert.xml` è su `@pec.fatturapa.it`, collegate con `NomeFile` e archiviate nello storage; niente doppioni (`SdiNotification.dedupeKey`); stati che vanno solo avanti; indirizzo assegnato imparato dal mittente della prima ricevuta ("verrà utilizzato anche dal SdI per i messaggi in risposta", Allegato B v1.8.4 §3.1.1, archiviato); Message-ID generato e salvato prima dell'invio, così un invio interrotto si chiude con la ricevuta di accettazione; lettura all'avvio, ogni 10 minuti (scelta dell'app) solo per le partite IVA con invii in attesa, e con "Controlla ricevute"; avvisi per invio non confermato e per nessun esito dopo 5 giorni. Test sugli esempi ufficiali RC, NS, MC dentro messaggi PEC costruiti secondo le regole tecniche.
   Fatto il 26/09/2026: import manuale delle ricevute SDI (RC, NS, MC) in XML o ZIP, anche insieme alle fatture: il portale le fornisce con "Consultazioni e download massivi", che "consente di scaricare [...] le ricevute dei file trasmessi" (AdE, "La home page del portale Fatture e Corrispettivi"; il "Monitoraggio delle ricevute" permette solo di visualizzarle). La ricevuta si collega alla fattura dal NomeFile; per le fatture inviate fuori dall'app si crea un invio con canale "altro strumento" (`SdiChannel.OTHER`). Import unico nel modulo `imports` (pattern Strategy con registro di gestori: fatture, poi ricevute). **Da verificare**: la struttura dello ZIP scaricato dal portale e se il nome dei file delle fatture scaricate coincide con il NomeFile delle ricevute.
   Fatto il 26/09/2026: una sola definizione di fattura emessa (`common/invoice-issue.ts`: consegnata, messa a disposizione o importata; con lo scarto "si considera non emessa", Spec. 1.9.1 §1.6): il fatturato conta solo le emesse, le scartate escono da proiezione delle soglie, bollo e Intrastat.
   Da fare: prova reale sulla casella.
   **Riconciliazione (l'app può restare spenta per giorni)**. Le ricevute non si perdono: lo SDI le inoltra "sullo stesso canale attraverso il quale sono state trasmesse le relative fatture" (Spec. 1.9.1 §1.5), cioè nella casella PEC, dove restano. Quindi niente ascolto in tempo reale, ma una sincronizzazione idempotente con la casella:
   - **Una sola sincronizzazione**, chiamata all'avvio dell'API, periodicamente con l'app aperta e dal pulsante "Controlla ricevute": dopo un fermo recupera tutto quello che è arrivato nel frattempo.
   - **Segnaposto IMAP per tenant**: `UIDVALIDITY` e ultimo UID letto della cartella; si leggono solo i messaggi con UID maggiore. Se `UIDVALIDITY` cambia, rilettura per data (`SINCE`) dal più vecchio invio senza esito.
   - **Idempotenza**: vincolo unico su `SdiNotification` (tipo + IdentificativoSdI + nome file, o Message-ID della PEC); rileggere lo stesso messaggio non crea doppioni né cambia due volte lo stato.
   - **Casella in sola lettura**: nessun messaggio spostato, cancellato o segnato come letto; il flag letto/non letto non fa da segnaposto (l'utente usa anche webmail e telefono).
   - **Correlazione**: le ricevute SDI riportano `NomeFile` e `IdentificativoSdI`; il nome file è unico per tenant e porta a `SdiTransmission` e alla fattura.
   - **Una sincronizzazione alla volta per tenant** (lock nel database), così il controllo periodico e il pulsante non si sovrappongono.
   - **Invii senza esito**: se la casella non ha la ricevuta (cancellata dalla webmail, casella piena, archiviazione del gestore) servono controlli a tempo. Scarto: la ricevuta arriva "entro 5 giorni dalla corretta ricezione del file" (Spec. 1.9.1 §1.6), quindi dopo 5 giorni senza ricevute SDI avviso di controllare sul portale Fatture e Corrispettivi. Consegna: via PEC lo SDI fa al massimo 2 tentativi, il secondo "almeno 40 ore dal primo tentativo" in assenza di notifica, poi "il processo si chiude" e lo stato si verifica sul portale (§1.5). Verificato il 26/09/2026: il §1.7 definisce solo T0 (ricezione da parte dello SDI, data della ricevuta di consegna del gestore PEC del mittente) e T1 (consegna al destinatario), senza durate massime. Per il destinatario il §1.5 dà: via PEC, ricevuta di impossibilità di recapito se il gestore non comunica nulla "entro un tempo massimo di 40 ore"; via SDICoop, "massimo di 4 tentativi di trasmissione, uno ogni 6 ore". Nessuna fonte fissa il tempo tra il superamento dei controlli e il primo tentativo di consegna: la soglia dell'avviso "consegna non ancora arrivata" è quindi una scelta dell'app (proposta: 7 giorni dall'invio), da dire così nell'interfaccia, non come termine di legge.
   - **Import manuale della ricevuta**: caricare l'XML scaricato dal portale o dalla casella, con lo stesso parser della sincronizzazione.
   - **Invio rimasto "in invio"**: se l'API cade dopo l'invio SMTP e prima di salvare lo stato, `SdiTransmission` resta `PENDING` e la fattura `SENT`. Dopo qualche minuto la sincronizzazione cerca nella casella la ricevuta di accettazione del gestore o una ricevuta SDI con quel nome file; se non trova niente lo segnala all'utente, senza reinviare da sola (meglio chiedere che rischiare un doppio invio). Verificato il 26/09/2026 sulle regole tecniche PEC (allegato al DM 2 novembre 2005, AgID, §6.3 e §7.4):
     - il gestore **sostituisce il Message-ID** messo dal nostro client: "Qualora il client di posta elettronica [...] avesse già inserito un Message ID [...] questo dovrà essere sostituito"; il nostro resta in `X-Riferimento-Message-ID`, "incluso all'interno delle ricevute e della busta di trasporto", e nell'elemento `msgid` di `daticert.xml` ("Message-ID del messaggio originale prima della modifica"). Quindi le ricevute del gestore si collegano all'invio con il `pecMessageId` già salvato in `SdiTransmission`, confrontato con `X-Riferimento-Message-ID` o `msgid`; l'identificativo del gestore (`identificativo` in `daticert.xml`) va salvato a parte.
     - Tipo di ricevuta nell'header `X-Ricevuta` (`accettazione`, `non-accettazione`, `avvenuta-consegna`, `errore-consegna`, `preavviso-errore-consegna`, `presa-in-carico`, `rilevazione-virus`) e nell'attributo `tipo` di `postacert`; i messaggi in arrivo sono buste di trasporto con `X-Trasporto: posta-certificata` e il messaggio originale in `postacert.eml`: le ricevute SDI (RC, NS, MC) vanno cercate lì dentro e collegate con `NomeFile`.
     - Archiviate il 26/09/2026 nel registro delle fonti: `agid-dm-2005-11-02-pec` (il decreto) e `agid-pec-regole-tecniche` (le regole tecniche allegate); agid.gov.it aggiunto ai domini ufficiali per la PEC.
...
7. Dipendenze: `nodemailer` 10.0.10 (SMTP), `imapflow` 2.0.7 (IMAP), `postal-mime` 3.0.1 (lettura dei messaggi) e `@nestjs/schedule` 12.0.2 (lettura periodica), installate.
   Archiviate il 26/09/2026 nel registro delle fonti: `fatturapa-spec-sdi-1-8-4` (Specifiche tecniche SdI, Allegato B DM 55/2013, v1.8.4: §3.1.1 invio e indirizzo assegnato, §3.2.1 ricezione) e `fatturapa-inviare-la-fatturapa` (pagina "Inviare la FatturaPA").
...
- Fatto il 26/09/2026: **"Segna come incassata"** tra le azioni di riga delle fatture emesse ("Registra rimborso" sulle note di credito). Apre una modale con data di oggi (fuso orario italiano), importo residuo e, per le fatture in valuta, il cambio del giorno dell'incasso (TUIR art. 9 c. 2) precompilato con quello della Banca d'Italia. Gli incassi si registrano solo da qui: il dettaglio della fattura mostra incassato, residuo e l'elenco degli incassi, con l'eliminazione.
- Da fare: stato "Incassata" o "Da incassare" nella lista, che richiede all'API l'importo incassato di ogni fattura (con DTO di risposta e mapper, vedi l'epica "Qualità del codice e architettura dell'API").
